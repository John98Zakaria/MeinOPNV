...
const client: HafasClient = createClient(profile, 'hafas-client-example');

const locations = await client.locations('Generalüberspekstraße mainz', {
    fuzzy: true,
    subStops: true,
    stops: true,
});
...
const journey = await client.journeys(kurmainKaserne, mainzGonsenheim, {
    results: 2,
});
